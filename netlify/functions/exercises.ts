...
const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
    ...
    //         "X-RapidAPI-Key": String(process.env.RAPID_KEY)
    //     }
    // };
    ...
    // while (true) {
    //     try {
    //         const url = `https://exercises-by-api-ninjas.p.rapidapi.com/v1/exercises?muscle=${event.queryStringParameters?.muscle ?? ""}&offset=${offset}`;
    //         const res = await fetch(url, options);
    //         if (res.status !== 200) {
    //             console.log(res);
    //             return {
    //                 statusCode: res.status,
    //                 body: JSON.stringify({status: res.body})
    //             }
    //         }
